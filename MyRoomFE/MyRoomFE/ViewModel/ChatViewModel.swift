...
class ChatViewModel: ObservableObject {
    ...
    func fetchChatRoomDetails(snapshot: DataSnapshot, completion: @escaping () -> Void) {
        print("fetchChatRoomDetails start currentUser : \(currentUser)")
        ...
        for (index, (roomId, roomName)) in data.enumerated() {
            ...
            db.child("chats/\(roomId)/").observeSingleEvent(of: .value) { snapshot in
                ...
                {
                    if !self.chatRooms.contains(where: { $0.id == roomId }) { //중복방지 위함
                        let chatRoom = ChatRoom(id: roomId, roomName: roomName, participants: participants)
    ...
    func fetchUserImages() {
        print("fetchUserImages start currentUser : \(currentUser)")
    ...
    func fetchLastMessages() {
        print("fetchLastMessages start currentUser : \(chatRooms)")
